...
- RIL may not work with other third-party tools that modify Reflex's `vite.config.js` file. As far as I know, 
RIL is the only such tool in existence, so I would be *shocked* if this created an actual problem for anyone. On the 
off chance this change breaks something for you, please [open an issue](https://github.com/celsiusnarhwal/RIL/issues) and explain your situation.
...
- Simple Icons' [/e/](https://simpleicons.org/?modal=icon&q=e) icon can now be passed as `"/e/"` (it previously could only be passed as `"e"`, without the slashes).
- The `color` props of the Simple Icons, Material Symbols, Octicons, Phosphor and Bootstrap Icons components now 
...
- Fixed a bug that prevented the simultaneous use of multiple versions of the same Simple Icons icon.
